...
public class AltaTrabajadorController implements Serializable{
    ...
    public void altaTrabajador(){
        try{
            if(!existeUsuario()){
            crearPersona();
             Rol tiporol = RolEJB.buscarRol("T");
            usuario.setPersona(persona);
            usuario.setRol(tiporol);
            usuarioEJB.create(usuario);
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "El trabajador se ha registrado correctamente"));
            }else{
